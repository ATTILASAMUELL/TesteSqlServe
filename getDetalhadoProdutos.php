...
try{
    ...
    if($method === 'get'){
        ...
        $desc_prod = filter_input(INPUT_GET, 'desc_prod');

        

        if($desc_prod){
            $sql = $pdo->prepare("SELECT * FROM produtoss WHERE desc_prod LIKE :desc_prod");
            $sql->bindValue(':desc_prod', $desc_prod."%", PDO::PARAM_STR);
            $sql->execute();
